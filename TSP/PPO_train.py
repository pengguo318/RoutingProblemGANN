...
def rollout(model, dataset,batch_size, n_nodes):
    ...
    def eval_model_bat(bat):
        with torch.no_grad():
            cost, _ = model.act(bat,0,n_nodes,batch_size,True,False)
...
def train():
    ...
    for lr, hidden_node_dim, hidden_edge_dim, epoch,batch_size,conv_laysers,entropy_value,eps_clip,timestep,ppo_epoch ,data_size,valid_size in runs:
        ...
        trainppo = TrainPPO(n_nodes,False,lr,2,hidden_node_dim,1,hidden_edge_dim, epoch,batch_size,conv_laysers,entropy_value,eps_clip,timestep,ppo_epoch)
